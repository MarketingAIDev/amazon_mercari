...
use Amazon\ProductAdvertisingAPI\v1\com\amazon\paapi5\v1\Merchant;
...
class MypageController extends Controller
{
	...
	public function AmazonAPI(Request $request)
	{
		return view('components.amazon_api');
	}
	...
	{
		//dd($request['	']);
		$sen = Setting::where('user_id', Auth::user()->id)->first();
		if (empty($request['sentence'])) {
			$sen->sentence = null;
		} else {
			$sen->sentence = $request['sentence'];
		}
		$sen->save();
	}
	...
	public function change_setting(Request $request)
	{
		...
		$setting->price_cut = ($request['price_cut'] == "true") ? 1 : 0;
		$setting->save();
		return $setting;
	}
	public function change_setting_price(Request $request)
	{
		$setting = Setting::where('user_id', Auth::user()->id)->first();
		$setting->etc = $request['etc'];
		$setting->save();
		return $setting;
	}
	public function change_price_reduction(Request $request)
	{
		$setting = Setting::where('user_id', Auth::user()->id)->first();
		$setting->price_reduction = $request['price_reduction'];
		$setting->save();
		return $setting;
	}
	public function change_setting_price_date(Request $request)
	{
		$setting = Setting::where('user_id', Auth::user()->id)->first();
		$setting->price_cut_date = $request['price_cut_date'];
		$setting->save();
		return $setting;
	}
	...
	public function save_postage(Request $request)
	{
		$req = json_decode($request['exData']);
		$postage = Postage::where('id', $req->id)->first();
		if ($postage == null) {
			$postage = new Postage;
		}
		$postage->user_id = Auth::id();
		$postage->size = $req->size;
		$postage->width = $req->width;
		$postage->length = $req->length;
		$postage->height = $req->height;
		$postage->final = $req->final;
		$postage->save();
		return $postage;
	}
	...
	public function price_remove()
	{
		...
		return redirect()->route('setting_price');
	}

	public function setting_postage(Request $request)
	{
		$postage_get = Postage::where('user_id', Auth::user()->id)->orderBy('id', 'desc')->paginate(10);
		return view('components.setting_postage', ['postage_get' => $postage_get]);
	}

	public function delete_postage($id)
	{
		Postage::where('id', $id)->delete();
		return redirect()->route('setting_postage');
	}
	...
	{
		Postage::where('user_id', Auth::id())->delete();
		return 'success';
	}
	...
	public function mercari_list_products(Request $request, $from, $to, $start, $end)
	{
	...
	public function mercari_update_list(Request $request, $from, $to, $start, $end)
	{
		...
		return view('components.mercari_update_list', ['mercari_update_data' => "[{$from},{$to}]"]);
	}
	...
	public function create_amazon_data(Request $request)
	{
		...
		for ($i = 0; $i < $length; $i++) {
			...
			$xlsxAmazon->image = ($xlsxAmazon->image) ?? $xlsxAmazonArr[$i]['画像'] ?? '';
			...
			$xlsxAmazon->feature = isset($xlsxAmazonArr[$i]['説明 & 機能: 説明']) ? json_encode($xlsxAmazonArr[$i]['説明 & 機能: 説明']) : '';
			$xlsxAmazon->feature_1 = isset($xlsxAmazonArr[$i]['説明 & 機能: 特徴 1']) ? json_encode($xlsxAmazonArr[$i]['説明 & 機能: 特徴 1']) : '';
			$xlsxAmazon->feature_2 = isset($xlsxAmazonArr[$i]['説明 & 機能: 特徴 2']) ? json_encode($xlsxAmazonArr[$i]['説明 & 機能: 特徴 2']) : '';
			$xlsxAmazon->feature_3 = isset($xlsxAmazonArr[$i]['説明 & 機能: 特徴 3']) ? json_encode($xlsxAmazonArr[$i]['説明 & 機能: 特徴 3']) : '';
			$xlsxAmazon->feature_4 = isset($xlsxAmazonArr[$i]['説明 & 機能: 特徴 4']) ? json_encode($xlsxAmazonArr[$i]['説明 & 機能: 特徴 4']) : '';
			$xlsxAmazon->feature_5 = isset($xlsxAmazonArr[$i]['説明 & 機能: 特徴 5']) ? json_encode($xlsxAmazonArr[$i]['説明 & 機能: 特徴 5']) : '';
			$xlsxAmazon->price = $xlsxAmazon->price ?? 0;
			$xlsxAmazon->r_price = $xlsxAmazon->r_price ?? 0;
	...
	public function create_matching_categories(Request $request)
	{
		...
		try {
			for ($i = 0; $i < $length; $i++) {
				$xlsxAmaon = Category::where('user_id',$user_id)->where('a_c_root',$req[$i]['カテゴリ: Root'])->where('a_c_sub',$req[$i]['カテゴリ: Sub'])->where('m_category',$req[$i]['メルカリカテゴリー'])->first();
				if($xlsxAmaon == null) {
	...
	public function delete_account(Request $request)
	{
		$id = $request->id;
		User::find($id)->delete();
		AmazonProduct::where('user_id', $id)->delete();
		MercariProduct::where('user_id', $id)->delete();
		MercariUpdate::where('user_id', $id)->delete();
		NgWord::where('user_id', $id)->delete();
		NgProduct::where('user_id', $id)->delete();
		NgCategory::where('user_id', $id)->delete();
		Exhibition::where('user_id', $id)->delete();
		Setting::where('user_id', $id)->delete();
		Category::where('user_id', $id)->delete();
		CategoryId::where('user_id', $id)->delete();
		Price::where('user_id', $id)->delete();
		Postage::where('user_id', $id)->delete();
	}
}
