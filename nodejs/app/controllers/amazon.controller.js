...
// const eachExhibition = async (amazonData, condition, ngCategory, ngWord, ngProduct, setting, criteria, query_condition, mercari_update) => {
...
// 				} else {
// 					query.condition_n_u = 0;// product is already exist mercari.
// 				}
// 			})
...
// 	}
// };
// const eachMercari = async (data, condition) => {
...
exports.saveExhibition = async (req, res) => {
	...
	if (amazonData.length != 0) {
		...
		for (let i = 0; i < amazonData.length; i++) {
			...
			// if ( exclusion == '' ) {
			// 	let string = '0000000';
			// 	m_code = 'MC' + string.substring( ( Math.floor( Math.log10( number ) ) + 1 ) ) + number;
			// 	number++;
			// }
			if (profit != null) {
				if (profit.profit == null && profit.profit == 0) {
		...
		// if (start_num < amazonData.length) {
		// 	const eachfunc = eachExhibition(amazonData.slice(start_num, (start_num + 1000)), condition, ngCategory, ngWord, ngProduct, setting, criteria, query_condition, mercari_update);
