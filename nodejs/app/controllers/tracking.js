...
const all_postage = postages.findAll();

exports.updateInfo = async () => {
	products.findAll({ group: ['ASIN'] })
		.then(res => {
			var index = 0;
			var len = res.length;
			var asins = [];
			...
			let checkInterval = setInterval(() => {
				if (index < len) {
					let checkItemInfo = new CheckItemInfo(asins.slice(index, (index + 10)));
					checkItemInfo.main();
					index += 10;
				} else {
...
class CheckItemInfo {
	...
	main() {
		const commonParameters = {
			AccessKey: 'AKIAI5IYF47G6SNOTWKA',
			SecretKey: 'I1F6OhfnpQh3kGcg+TphnBKr+u2+sLb/MFpYhRYn',
			PartnerTag: 'kazuya1984007-22', // yourtag-20
						...
						await products.findAll({ where: condition })
							.then(async (datas) => {
								...
								datas.forEach(async ele => {
									await mercariUpdates.update(mercari_update_info, { where: { SKU1_product_management_code: ele.m_code } });
								});
